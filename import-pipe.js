...
var pipe2js = {
    ...
    fetchPipe: function (pipeId, cb) {
	   if(pipeId.indexOf('.json')>-1){
	        var loaded = JSON.parse(fs.readFileSync(pipeId, 'utf8'));	
            console.log(loaded);	
		    cb(JSON.parse(loaded.PIPE.working));	
        } else {
		    ...
		    if (!error && response.statusCode === 200) {
			  ...
			  console.log("Pipes is dead and it's never coming back.");
		    }
		});
	}
    },
